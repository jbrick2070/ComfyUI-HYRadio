...
WORKFLOW_PATH = r"C:\Users\jeffr\Documents\ComfyUI\custom_nodes\ComfyUI_HYWorld2\workflows\otr_scifi_16gb_full.json"
SOAK_LOG = r"C:\Users\jeffr\Documents\ComfyUI\custom_nodes\ComfyUI_HYWorld2\logs\soak_log.md"
RUNTIME_LOG = r"C:\Users\jeffr\Documents\ComfyUI\custom_nodes\ComfyUI_HYWorld2\otr_runtime.log"
OUTPUT_DIR = r"C:\Users\jeffr\Documents\ComfyUI\output\hyradio"
# Watcher-owned overrides file. Read at the top of every iteration. The
# hyradio-alert-watcher scheduled task may write this to pin parameters,
...
WATCHER_OVERRIDES_PATH = r"C:\Users\jeffr\Documents\ComfyUI\custom_nodes\ComfyUI_HYWorld2\scripts\watcher_overrides.json"
...
# Node 1 (HYRadio_LLMScriptWriter) widgets_values index map:
...
# NTFY PUSH ALERTS
# ---------------------------------------------------------------------------
# Public ntfy.sh server, topic "hyradio_alert". Subscribe on phone via
# the ntfy app (search the topic) or at https://ntfy.sh/hyradio_alert .
NTFY_URL = "https://ntfy.sh/hyradio_alert"
NTFY_TIMEOUT_S = 5
...
def send_ntfy_alert(title, message, priority="default", tags=None):
    """Fire-and-forget push alert to ntfy.sh/hyradio_alert.
...
def reboot_comfyui():
    ...
    send_ntfy_alert(
        "HYRadio Soak: ComfyUI reboot stalled",
...
VIDEO_IDEAS = [
    ...
    "Add a retro TV static transition between scenes -- fits the vintage radio aesthetic perfectly.",
...
def run_iteration(run_num):
    ...
    try:
        ...
        if not comfyui_alive():
            ...
            send_ntfy_alert(
                f"HYRadio Soak STOPPED (run {run_num:03d})",
        ...
        for node in workflow.get("nodes", []):
            if node.get("id") == 1 and node.get("type") == "HYRadio_LLMScriptWriter":
    ...
    if result in ("FAIL", "TIMEOUT"):
        send_ntfy_alert(
            f"HYRadio Soak run {run_num:03d} {result}",
            (f"Episode: {title}\n"
             f"Duration: {duration}s\n"
             f"Dialogue lines: {dialogue}\n"
             f"VRAM peak: {vram} GB\n"
    ...
    if result == "SUCCESS":
        ...
        if newest:
            ...
            if scan_flags:
                ...
                send_ntfy_alert(
                    f"HYRadio Soak run {run_num:03d}: treatment scan flags",
                    (f"Episode: {title}\n"
                     f"File: {os.path.basename(newest)}\n"
...
def trigger_fatal_halt(run_num, tag):
    ...
    append_to_log(f"\n--- {reason} ---\n")
    send_ntfy_alert(
        "HYRadio Soak HALTED (fatal streak)",
        reason,
        priority="urgent",
        tags=["skull", "rotating_light"],
    )
...
def wait_for_stop_clear(run_num):
    ...
    append_to_log(f"\n--- {msg} ---\n")
    send_ntfy_alert(
        "HYRadio Soak paused (stop file present)",
        msg,
        priority="default",
        tags=["pause_button"],
    )
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            send_ntfy_alert(
                f"HYRadio Soak CRASH (run {run_num:03d})",
