"""
Regression tests for ComfyUI_HYWorld2 — "SIGNAL LOST"
...
class TestWorkflowJSONFull:
    ...
    @pytest.fixture(scope="class")
    def wf(self):
        return _load_workflow("HYRadio-Full-Pipeline.json")
    ...
    def test_node_types_otr_or_known(self, wf):
        ...
        for n in wf["nodes"]:
            valid = (
                n["type"].startswith("HYRadio_") or
                n["type"].startswith("HYWorld_") or
                n["type"].startswith("VNCCS_") or
                n["type"] in known
            )
            assert valid, f"Unknown node type: {n['type']}"

    def test_required_pipeline_nodes(self, wf):
        types = {n["type"] for n in wf["nodes"]}
        required = {
            "HYRadio_LLMScriptWriter", "HYRadio_LLMDirector",
            "HYRadio_BatchBarkGenerator", "HYRadio_SceneSequencer",
            "HYRadio_EpisodeAssembler",
        }
...
class TestVRAMGuardianNode:
    """Verify HYRadio_VRAMGuardian node is importable and structurally correct."""
    ...
    def test_category(self):
        from nodes.vram_guardian import VRAMGuardian
        assert VRAMGuardian.CATEGORY == "HYRadio"
...
class TestParserV3V4Patterns:
    ...
    def test_no_traits_inline(self, parser):
        """[VOICE: NAME] dialogue — no traits specified."""
