...
_WORKFLOW = os.path.join(_HERE, "..", "..", "workflows", "HYRadio-Full-Pipeline.json")

# Fixed seed for deterministic audio output.
# These override the workflow's randomized seeds at runtime.
FIXED_SEEDS = {
    "HYRadio_LLMScriptWriter": 42,
    "HYRadio_LLMDirector": 42,
    "HYRadio_BatchBarkGenerator": 42,
    "HYRadio_KokoroAnnouncer": 42,
    "HYRadio_BatchAudioGenGenerator": 42,
    "HYRadio_MusicGenTheme": 42,
}
...
class TestWorkflowSeedContract:
    ...
    def test_episode_assembler_present(self):
        ...
        assert "HYRadio_EpisodeAssembler" in types
...
class TestAudioRegressionGate:
    ...
    @pytest.mark.skipif(not _HAS_BASELINE, reason=_SKIP_MSG)
    @pytest.mark.skipif(
        not os.environ.get("HYRadio_REGRESSION_RUNTIME"),
        reason=(
            "Set HYRadio_REGRESSION_RUNTIME=1 to run full audio regression. "
