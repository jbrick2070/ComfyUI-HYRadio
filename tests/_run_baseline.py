...
Executes the full HYRadio workflow via ComfyUI's HTTP API with fixed seeds,
...
log = logging.getLogger("HYRadio.baseline")
...
_WORKFLOW_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "workflows", "HYRadio-Full-Pipeline.json"
)
...
# visual nodes have been removed (sidecar architecture pending).
# These nodes are just dummies in the baseline workflow to keep routing intact.
_PLACEHOLDER_TYPES = set()
...
# Known-correct widget values for HYRadio_BatchAudioGenGenerator.
...
def _fix_known_widget_drift(prompt):
    ...
    BUG-LOCAL-008: HYRadio_BatchAudioGenGenerator gets episode_seed and
...
def _strip_non_audio_nodes(prompt):
    ...
    to_remove = [
        ...
        if data.get("class_type") in _PLACEHOLDER_TYPES
    ]
    for nid in to_remove:
        del prompt[nid]
        log.info("Stripped non-audio node #%s from prompt", nid)
    if to_remove:
        print(f"  Stripped {len(to_remove)} placeholder nodes")
    return prompt
...
def _find_assembler_node_id(prompt):
    ...
    for nid, node_data in prompt.items():
        if node_data.get("class_type") == "HYRadio_EpisodeAssembler":
            return nid
    raise RuntimeError("HYRadio_EpisodeAssembler not found in workflow prompt")
...
def run_episode_and_get_audio_bytes(seeds):
    ...
    _dump_schema_for_node(
        schemas, "HYRadio_BatchAudioGenGenerator",
        os.path.join(os.path.dirname(__file__), "debug_audiogen_schema.json")
    )

    print("Stripping non-audio placeholder nodes...")
