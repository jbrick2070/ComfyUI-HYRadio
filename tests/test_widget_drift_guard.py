...
def _legacy_drift_schema():
    ...
    opt = schema["HYRadio_LLMScriptWriter"]["input"]["optional"]
    reordered = {}
    for k, v in opt.items():
        if k == "optimization_profile":
            ...
            reordered[k] = v
    schema["HYRadio_LLMScriptWriter"]["input"]["optional"] = reordered
    return schema
...
class TestLinkedConvertedWidgetSlots:
    ...
    Canonical failure: Node 15 (HYRadio_BatchAudioGenGenerator) crashes with
    ...
    def _node15_schema(self):
        return {
            "HYRadio_BatchAudioGenGenerator": {
                "input": {
                    "required": {
    ...
    def _node15_workflow(self):
        """Mirrors the on-disk shape in HYRadio-Full-Pipeline.json where
        ...
        return {
            ...
            "nodes": [
                {
                    "id": 15,
                    "type": "HYRadio_BatchAudioGenGenerator",
...
class TestPreservedSlotMode:
    ...
    stripped. Node 2 (HYRadio_LLMDirector) in HYRadio-Full-Pipeline.json
    ...
    def _director_workflow_preserved(self):
        """Shape of Node 2 in HYRadio-Full-Pipeline.json.
