r"""
HYRadio Orchestrator - Script Writer + Director for "SIGNAL LOST"
===================================================================
...
def _run_with_timeout(fn, timeout_sec, phase_label="LLM"):
    ...
    executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix=f"hyradio-{phase_label}")
...
def _fetch_full_article(url, timeout=20):
    ...
    try:
        headers = {"User-Agent": "Mozilla/5.0 (compatible; HYRadio-ScriptBot/1.0)"}
...
def _fetch_science_news(max_feeds=10):
    ...
    try:
        ...
        msg = (
            ...
            "-  The HYRadio ScriptWriter REQUIRES real headlines - no fallback.   -\n"
            "-==================================================================-"
        )
        log.error(msg)
    ...
    if not pool:
        ...
        raise RuntimeError(
            ...
            "The HYRadio ScriptWriter requires live RSS feeds to generate scripts."
        )
...
# â”€â”€ Token Budget Ratios â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#
# Breakdown for dialogue-dominant HYRadio scripts:
#   tokenizer overhead:  ~1.3 tokens per English word
#   script markup:       ~1.2x (VOICE/SFX/ENV tags, scene headers, beats)
#   combined:            1.3 * 1.2 = 1.56 â†’ round to 1.6
#
# Revision/rewrite passes include structural reorganization â†’ higher overhead.
# Outlines and pitches are almost entirely non-dialogue description.
_TOKEN_RATIO_DIALOGUE = 1.6    # dialogue-dominant (HYRadio radio drama default)
...
# â”€â”€ Intelligent Dialogue Name Normalizer (BUG-023) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#
# Handles:
#   **FIRST LAST**, concerned: text    â†’ FIRST LAST: text
#   *FIRST LAST*(angry): text          â†’ FIRST LAST: text
#   __FIRST LAST__: text               â†’ FIRST LAST: text
#   FIRST_LAST: text                   â†’ FIRST LAST: text
#   *FIRST_LAST*, whispering: text     â†’ FIRST LAST: text
#
# Standard NAME: lines pass through unchanged (regex only fires on decorated
# names â€” plain uppercase + colon is a no-op match that rewrites identically).
...
def _normalize_dialogue_names(text):
    """Intelligent LLM output normalizer â€” strips all creative formatting
...
# â”€â”€ Scene inventory (diagnostic instrumentation) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Name cleanup (fuzzy match against canonical cast) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Dual-format dialogue extraction â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class GemmaHeartbeatStreamer(BaseStreamer):
    ...
    The HYRadio Monitor (hyradio_monitor.py) tails hyradio_runtime.log and folds these
    ...
    def write_script(self, episode_title, genre_flavor,
        ...
        if custom_premise:
            ...
            for n in news:
                ...
                if len(_raw) < 500:
                    # Short text â€” no summarization needed
                    continue
                _runtime_log(
                ...
                _summary_prompt = (
                    ...
                    "- Keep quotes from researchers or officials â€” these become character dialogue\n"
                    ...
                    "- Remove repetitive phrasing â€” say each fact exactly once\n"
                ...
                try:
                    ...
                    log.warning("[NEWS_SUMMARY] Summarization failed: %s â€” keeping original text", _e)
                    if len(_raw) > 12000:
                        n["full_text"] = _raw[:12000] + "\n[... article truncated at 12,000 chars]"
        ...
        if is_small_model:
            ...
            lite_role = "<system_role>STRICT HYRadio TAGS ONLY. No prose. Start every line with a tag.</system_role>"
...
[MUSIC: Closing theme]"""
        ...
        #
        # Order matters:
        #   1. WORD_EXTEND  â€” count dialogue words, extend if under 70%
        #   2. ANNOUNCER    â€” add bookends (sees full extended script)
        #   3. FORMAT_NORM  â€” clean up everything into canonical format
        #   4. PARSE        â€” parse clean text into structured JSON
        # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        ...
        _target_words = target_words  # Direct from widget â€” no conversion needed
        ...
        # BUG-024: Zero-dialogue detection â€” creative generation produced
        ...
        # -- STEP 3: FORMAT NORMALIZER (Creative â†’ Strict) ------------
...
- Develop character relationships â€” disagreements, alliances, secrets
...
- Do NOT write scene headers, SFX, or ENV tags â€” ONLY dialogue lines
...
OUTPUT ONLY THE NEW DIALOGUE LINES, one per line:"""
        ...
        try:
            ...
            # Filter extension output â€” only keep valid dialogue lines
            valid_lines = []
...
"""
    ...
    def _parse_script(self, text):
        ...
        # HYRadio Canonical 1.0 RegEx Patterns
