...
Replaces the previous "no music at all" hole in the HYRadio pipeline. Reads the
...
Design notes (see ROADMAP v1.4 Theme A):
  - NO audiocraft dependency. Uses transformers.MusicgenForConditionalGeneration
    and AutoProcessor - both already installed in the HYRadio venv via the main
...
class MusicGenTheme:
    """HYRadio v1.4 - instrumental music generator for opening, closing, and
    act-break interstitial cues.
