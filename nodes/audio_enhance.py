"""
HYRadio_AudioEnhance - Broadcast-quality spatial audio enhancement.
...
log = logging.getLogger("HYRadio.AudioEnhance")
...
class AudioEnhance:
    ...
    def enhance(self, audio, target_sample_rate=48000, spatial_width=0.3,
        ...
        if waveform is None:
            log.warning("[HYRadio_AudioEnhance] No waveform data - passing through")
            return (audio,)
        ...
        log.info("[HYRadio_AudioEnhance] Input: %dHz %dch %d samples (%.1fs)",
        ...
        if lpf_cutoff_hz > 0 and lpf_cutoff_hz < target_sample_rate / 2:
            ...
            log.info("[HYRadio_AudioEnhance] LPF at %.0fHz applied (chirp cleanup)",
                     lpf_cutoff_hz)

        # -- Step 5: Haas spatial delay --
        if haas_delay_ms > 0:
            waveform = _haas_delay(waveform, target_sample_rate, haas_delay_ms)
            log.info("[HYRadio_AudioEnhance] Haas delay %.1fms applied", haas_delay_ms)
        ...
        if tape_emulation != "off":
            ...
            log.info("[HYRadio_AudioEnhance] Tape emulation '%s' applied", tape_emulation)

        # -- Step 6: Mid-side stereo widening --
        if spatial_width > 0:
            waveform = _stereo_decorrelate(waveform, spatial_width)
            log.info("[HYRadio_AudioEnhance] Stereo width %.2f applied", spatial_width)
        ...
        log.info("[HYRadio_AudioEnhance] Normalize deferred to EpisodeAssembler (post-crossfade)")
        ...
        assert waveform.shape[1] == 2, (
            f"[HYRadio_AudioEnhance] BUG: expected 2 channels, got {waveform.shape[1]}"
        )
        ...
        log.info("[HYRadio_AudioEnhance] Output: %dHz %dch %d samples (%.1fs) "
