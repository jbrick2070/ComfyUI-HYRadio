r"""
HYRadio_SignalLostVideo - Procedural Audio-Reactive Video Engine
=============================================================
...
try:
    ...
    raise ImportError(
        "Pillow is required for HYRadio_SignalLostVideo. "
...
class _CRTRenderer:
    ...
    def render(self, fi, total, fps, vol, freq, wave):
        ...
        draw.text((pad, by - pad // 6),
                  f"HYRadio v1.0  x  {self.w}x{self.h}  x  {fps}fps",
...
class _TelemetryHUDRenderer:
    ...
    def _build_left(self):
        ...
        d.text((P, fy), "HYRadio v1.0", fill=CRT_DARK, font=self.f_small)
        return img
...
class SignalLostVideoRenderer:
    """Generate a procedural CRT-aesthetic MP4 from an HYRadio episode."""
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "bypass_render": ("BOOLEAN", {
                    "default": False,
                    "tooltip": "Skip rendering and load a pre-rendered test video (saves time during structural tests)"
                }),
            },
        }
    ...
    def render_video(self, audio, script_json, production_plan_json,
                     ...
                     episode_title="", closing_audio=None, bypass_render=False):

        from .story_orchestrator import _runtime_log
        
        if bypass_render:
            _runtime_log("Video: BYPASS RENDER ACTIVE. Returning pre-rendered test file.")
            out_path = os.path.join(
                os.path.expanduser("~"), "Documents", "ComfyUI",
                "ucla_daily_test.mp4"
            )
            return {
                "ui": {"gifs": [{"filename": "ucla_daily_test.mp4",
                                 "subfolder": "",
                                 "type": "output"}]},
                "result": (out_path,),
            }
